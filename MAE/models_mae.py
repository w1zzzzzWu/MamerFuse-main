...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_return_feature(self, imgs, mask):
        ...
        image, new_mask = self.forward_return_image(imgs, mask)
        s = int(np.sqrt(new_mask.shape[1]))
        new_mask = new_mask.reshape(-1, s, s).unsqueeze(1).float()  # [N, 1, s, s]
        new_mask = F.interpolate(new_mask, size=imgs.shape[-2:], mode='nearest')  

        newmasked = imgs * (1 - new_mask) + new_mask 

        reco = image*new_mask+imgs*(1 - new_mask)


        newmasked = newmasked.detach().cpu()
        reco = reco.detach().cpu()

        save_dir = '/home/student01/MxT-main/MAEimg/'
        os.makedirs(save_dir, exist_ok=True)

        for i in range(newmasked.size(0)):
            filename = "maetest"
            save_path_masked = os.path.join(save_dir, filename + '_newmasked.png')
            save_path_reco = os.path.join(save_dir, filename + '_reco.png')
            save_path_ret = os.path.join(save_dir, filename + '_ret.png')
            self.save_tensor_as_image(newmasked[i], save_path_masked)
            self.save_tensor_as_image(reco[i], save_path_reco)
            self.save_tensor_as_image(image[i], save_path_ret)
    ...
    def save_tensor_as_image(self, tensor, save_path):

        tensor = tensor.detach().cpu().clamp(0, 1)  
        array = tensor.numpy()  # [C, H, W]
